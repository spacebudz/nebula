import { applyParamsToScript, Data, fromText, fromUnit, paymentCredentialOf, toLabel, toUnit, } from "../../deps.js";
import scripts from "./ghc/scripts.js";
import { fromAddress, fromAssets, sortAsc, sortDesc, toAddress, toAssets, } from "../../common/utils.js";
...
export class Contract {
    ...
    constructor(lucid, config = budConfig) {
        ...
        this.tradeValidator = {
            type: "PlutusV2",
            script: applyParamsToScript(scripts.trade, [
    ...
    async list(assetName, lovelace, privateListing) {
        ...
        const adjustedTradeAddress = stakeCredential
            ? this.lucid.utils.credentialToAddress(this.lucid.utils.scriptHashToCredential(this.tradeHash), stakeCredential)
            : this.tradeAddress;
        const tradeDatum = {
            Listing: [
                {
        ...
        const tx = await this.lucid.newTx().payToContract(adjustedTradeAddress, {
            inline: Data.to(tradeDatum, D.TradeDatum),
    ...
    async changeListing(listingUtxo, lovelace, privateListing) {
        const tradeDatum = await this.lucid.datumOf(listingUtxo, D.TradeDatum);
        if (!("Listing" in tradeDatum)) {
            throw new Error("Not a listing UTxO");
        }
        const listingDetails = tradeDatum.Listing;
        const owner = toAddress(listingDetails[0].owner, this.lucid);
        ...
        const tx = await this.lucid.newTx()
            .collectFrom([listingUtxo], Data.to("Cancel", D.TradeAction))
    ...
    async bid(assetName, lovelace) {
        ...
        const tx = await this.lucid.newTx()
            ...
            inline: Data.to(biddingDatum, D.TradeDatum),
        }, {
            lovelace,
            [toUnit(this.mintPolicyId, fromText("Bid") + assetName)]: 1n,
        })
            ...
            Bid: [{
                    ...
                    requestedOption: {
                        SpecificSymbolWithConstraints: [
                            ...
                            constraints?.traits
                                ? constraints.traits.map(({ negation, trait }) => [negation ? -1n : 0n, fromText(trait)])
                                : [],
                        ],
                    },
                }],
    ...
    async changeBid(bidUtxo, lovelace) {
        const tradeDatum = await this.lucid.datumOf(bidUtxo, D.TradeDatum);
        ...
        if (owner !== address)
            throw new Error("You are not the owner.");
        const refScripts = await this.getDeployedScripts();
        const tx = await this.lucid.newTx().collectFrom([bidUtxo], Data.to("Cancel", D.TradeAction)).payToContract(bidUtxo.address, {
    ...
    async getListings(assetName) {
        return (await this.lucid.utxosAtWithUnit(paymentCredentialOf(this.tradeAddress), toUnit(this.config.policyId, assetName))).filter((utxo) => Object.keys(utxo.assets).length === 2).sort(sortAsc);
    }
    /**
     ...
     */
    async getBids(assetName) {
        return (await this.lucid.utxosAtWithUnit(paymentCredentialOf(this.tradeAddress), toUnit(this.mintPolicyId, assetName === "Open"
            ? fromText("OpenBid")
            : fromText("Bid") + assetName))).filter((utxo) => Object.keys(utxo.assets).length === 2).sort(sortDesc);
    }
    /**
     ...
     */
    static async createRoyalty(lucid, royaltyRecipients, owner, minAda = 1000000n) {
        ...
        const royaltyMintingPolicy = {
            type: "PlutusV2",
            script: applyParamsToScript(scripts.oneShot, [
                {
        ...
        const tx = await lucid.newTx()
            ...
            .mintAssets({
            [royaltyUnit]: 1n,
        }, Data.void()).payToAddressWithData(ownersAddress, { inline: Data.to(royaltyInfo, D.RoyaltyInfo) }, { [royaltyUnit]: 1n })
     ...
     * minAda is the threshold that decides to pay fee as percentage or fixed.
     */
    async updateRoyalty(royaltyRecipients, minAda = 1000000n) {
    ...
    async _cancelListing(listingUtxo) {
        ...
        if (owner !== address)
            throw new Error("You are not the owner.");
        const refScripts = await this.getDeployedScripts();
        return this.lucid.newTx().collectFrom([listingUtxo], Data.to("Cancel", D.TradeAction))
            .addSigner(owner)
            .compose(refScripts.trade
            ? this.lucid.newTx().readFrom([refScripts.trade])
            : this.lucid.newTx().attachSpendingValidator(this.tradeValidator));
    ...
    async _sell(bidUtxo, assetName) {
        ...
        const bidToken = Object.keys(bidUtxo.assets).find((unit) => unit.startsWith(this.mintPolicyId));
        if (!bidToken)
        ...
        const { requestedAssets, refNFT } = (() => {
            ...
            else if ("SpecificSymbolWithConstraints" in bidDetails.requestedOption &&
                assetName) {
                const policyId = bidDetails.requestedOption.SpecificSymbolWithConstraints[0];
                const refNFT = toUnit(policyId, fromUnit(toUnit(policyId, assetName)).name, 100);
                const types = bidDetails.requestedOption.SpecificSymbolWithConstraints[1];
                const traits = bidDetails.requestedOption.SpecificSymbolWithConstraints[2];
                return {
                    requestedAssets: {
        ...
        const refScripts = await this.getDeployedScripts();
        return this.lucid.newTx().collectFrom([bidUtxo], Data.to("Sell", D.TradeAction))
            .compose(refNFT
            ? await (async () => {
            ...
            : null)
            .compose((await this._payFee(lovelace, paymentDatum)).tx).payToAddressWithData(owner, {
            inline: paymentDatum,
        }, requestedAssets)
    ...
    async _cancelBid(bidUtxo) {
        ...
        if (owner !== address)
            throw new Error("You are not the owner.");
        const [bidToken] = Object.keys(bidUtxo.assets).filter((unit) => unit !== "lovelace");
        const refScripts = await this.getDeployedScripts();
        return this.lucid.newTx().collectFrom([bidUtxo], Data.to("Cancel", D.TradeAction))
            .mintAssets({ [bidToken]: -1n })
            .validFrom(this.lucid.utils.slotToUnixTime(1000))
            .addSigner(owner)
            .compose(refScripts.trade
    ...
    async _buy(listingUtxo) {
        ...
        const refScripts = await this.getDeployedScripts();
        return this.lucid.newTx().collectFrom([listingUtxo], Data.to("Buy", D.TradeAction))
            .compose(await (async () => {
